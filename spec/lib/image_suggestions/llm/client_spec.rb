  ...
  describe "#call" do
    ...
    context "when title and description are blank" do
      ...
      it "adds error and returns early" do
        result = client.call
        expect(result.errors).to include "To suggest images, please write a title and description first."
    ...
    context "when LLM response has surrounding whitespace" do
      before do
    ...
    context "when provider is missing" do
      ...
      it "adds error to response and returns early" do
        ...
        expect(result.errors).to include("LLM is not properly configured. Please contact an administrator.")
        expect(result.results).to be_empty
      end
    ...
    context "when use_ai_image_suggestions is disabled" do
      before { Setting["llm.use_ai_image_suggestions"] = false }
